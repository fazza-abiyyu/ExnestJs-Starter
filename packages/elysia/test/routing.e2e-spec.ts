...
class TestController {
  ...
  list(ctx: HandlerContext) {
    return ODataResponse.collection([
      { page: ctx.query.page, tenant: ctx.headers['x-tenant-id'] },
    ]).build();
  }

  secure(_ctx: HandlerContext) {
...
describe('mountRoutes (routing + validation + permissions)', () => {
  ...
  test('runs permission handlers and returns 403 when denied', async () => {
    ...
    const res = await buildApp([forbidden]).handle(new Request('http://localhost/test/secure'));
  ...
  test('allows request when permission passes', async () => {
    ...
    const res = await buildApp([allow]).handle(new Request('http://localhost/test/secure'));
