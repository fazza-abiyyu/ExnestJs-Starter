...
describe('CustomersService', () => {
  ...
  describe('createCustomer', () => {
    ...
    test('should throw ODataError 409 when duplicate', async () => {
      ...
      expect(
        service.createCustomer('tenant-1', { name: 'Test Customer' }),
      ).rejects.toMatchObject({ code: 'CustomerDuplicate', status: 409 })
    })
  })
  ...
  describe('updateCustomer', () => {
    ...
    test('should throw ODataError 404 when not found', async () => {
      ...
      expect(
        service.updateCustomer('tenant-1', 'missing', { name: 'X' }),
      ).rejects.toMatchObject({ code: 'CustomerNotFound', status: 404 })
    })
  })
  ...
  describe('archiveCustomer', () => {
    ...
    test('should throw ODataError 404 when not found', async () => {
      ...
      expect(
        service.archiveCustomer('tenant-1', 'missing'),
      ).rejects.toMatchObject({ code: 'CustomerNotFound', status: 404 })
    })
  })
})