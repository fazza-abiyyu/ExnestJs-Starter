...
function assertPublicTarget(rawUrl: string): URL {
  ...
  try {
    ...
    throw new PdfEngineError('INVALID_URL', 'Invalid URL', 400);
  }
  if (url.protocol !== 'http:' && url.protocol !== 'https:') {
    throw new PdfEngineError('SSRF_BLOCKED', 'Only http(s) URLs are allowed', 400);
  }
  if (url.username || url.password) {
    throw new PdfEngineError('SSRF_BLOCKED', 'URLs with credentials are not allowed', 400);
  }
  return url;
}
...
export class SsrfGuard {
  ...
  async assertSafeUrl(rawUrl: string): Promise<URL> {
    ...
    try {
      ...
      throw new PdfEngineError('DNS_FAILED', 'Could not resolve host', 400);
    }
    if (records.length === 0) {
      throw new PdfEngineError('DNS_FAILED', 'Could not resolve host', 400);
    }
    if (records.some((r) => isPrivateAddress(r.address))) {
      throw new PdfEngineError('SSRF_BLOCKED', 'URL resolves to a private address', 400);
    }
    return url;
  }
}
