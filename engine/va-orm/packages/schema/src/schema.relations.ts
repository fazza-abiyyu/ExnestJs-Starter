...
import type { ModelBlock, RelationFieldInfo, RelationKind } from './schema.types.js';
...
export function resolveRelations(
  ...
  for (const model of models) {
    for (const field of model.fields) {
      ...
      const onDelete =
        args['onDelete'] !== undefined ? stripQuotes(String(args['onDelete'])) : undefined;
...
function linkBackReferences(relations: Map<string, RelationFieldInfo>): void {
...
function detectImplicitManyToMany(relations: Map<string, RelationFieldInfo>): void {
  for (const info of relations.values()) {
