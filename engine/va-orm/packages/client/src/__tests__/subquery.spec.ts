...
describe('SubqueryBuilder', () => {
  ...
  describe('fromRaw', () => {
    it('should create subquery from raw SQL', () => {
      const { sql, params } = subquery.fromRaw('SELECT id FROM users WHERE status = $1', [
        'active',
      ]);
  ...
  describe('aggregate subqueries', () => {
    it('should build count subquery', () => {
      ...
      expect(result).toBe(
        '(SELECT COUNT(*) FROM (SELECT * FROM posts WHERE userId = $1) AS _subquery)',
      );
    });
    ...
    it('should build sum subquery', () => {
      ...
      expect(result).toBe(
        '(SELECT SUM("amount") FROM (SELECT amount FROM transactions WHERE userId = $1) AS _subquery)',
      );
    });
    ...
    it('should build avg subquery', () => {
      ...
      expect(result).toBe(
        '(SELECT AVG("score") FROM (SELECT score FROM scores WHERE userId = $1) AS _subquery)',
      );
    });
    ...
    it('should build min subquery', () => {
      ...
      expect(result).toBe(
        '(SELECT MIN("price") FROM (SELECT price FROM products WHERE categoryId = $1) AS _subquery)',
      );
    });
    ...
    it('should build max subquery', () => {
      ...
      expect(result).toBe(
        '(SELECT MAX("price") FROM (SELECT price FROM products WHERE categoryId = $1) AS _subquery)',
      );
    });
  });
