...
describe('VaError redaction (M9)', () => {
  it('redacts DSN passwords', () => {
    const msg = VaError.redactSecrets('connect failed postgresql://user:secret@host:5432/db');
    expect(msg).not.toContain('secret');
    expect(msg).toContain('***');
  });

  it('wrap keeps code and redacts', () => {
    const err = VaError.wrap(new Error('auth failed for mysql://u:p@h/db'), 'mysql connect');
...
describe('typed joinOn (M10)', () => {
  ...
  it('joinOn quotes breakout in column names', () => {
    ...
    expect(sql).toBe('SELECT * FROM "users" INNER JOIN "posts" ON "id"" or 1=1 --" = "userid"');
  });
});
