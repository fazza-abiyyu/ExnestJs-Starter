...
describe('CTEBuilder', () => {
  ...
  describe('with', () => {
    it('should add CTE', () => {
      ...
      expect(sql).toBe(
        'WITH active_users AS (SELECT * FROM users WHERE status = $1) SELECT * FROM active_users',
      );
    ...
    it('should add multiple CTEs', () => {
      ...
      const { sql, params } = cte
        .select('SELECT * FROM active_users JOIN admin_users ON active_users.id = admin_users.id')
        .build();
  ...
  describe('build', () => {
    ...
    it('should build with CTE and main query params', () => {
      ...
      expect(sql).toBe(
        'WITH active_users AS (SELECT * FROM users WHERE status = $1) SELECT * FROM active_users WHERE id = $2',
      );
