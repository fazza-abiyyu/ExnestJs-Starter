...
describe('Migration', () => {
  ...
  describe('Migrator', () => {
    ...
    it('rejects path traversal in migration name (CRITICAL regression)', async () => {
      ...
      await expect(testMigrator.createMigration('../../tmp/evil')).rejects.toThrow(
        /Invalid migration name/,
      );
      await expect(testMigrator.createMigration('foo/bar')).rejects.toThrow(
        /Invalid migration name/,
      );
      await expect(testMigrator.resolve('../escape', 'applied')).rejects.toThrow(
        /Invalid migration name/,
      );
    });

    it('rejects path escape via resolveWithinBase', async () => {
      const { resolveWithinBase, assertSafeMigrationName } =
        await import('../migration/migrator.js');
      expect(() => assertSafeMigrationName('..')).toThrow(/Invalid migration name/);
      expect(() => resolveWithinBase('./migrations', '../etc/passwd')).toThrow(
        /Invalid migration name/,
      );
    ...
    it('should run dev migration (create + apply)', async () => {
      ...
      try {
        ...
        const { file, applied } = await testMigrator.dev(
          'add_users',
