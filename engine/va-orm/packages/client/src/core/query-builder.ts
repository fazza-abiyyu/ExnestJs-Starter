...
export class QueryBuilder<T = any> {
  ...
  where(fn: (builder: ExpressionBuilder) => void): this {
    const builder = ExpressionBuilder.create(this._placeholderFn, (name) =>
      quoteColumn(this.driver, name),
    );
    fn(builder);
  ...
  build(): { sql: string; params: any[] } {
    ...
    if (this._having.length > 0) {
      ...
      for (const clause of this._having) {
        if (isRawSql(clause.value)) {
          const processed = clause.value.raw.replace(/\?/g, () =>
            this._placeholderFn(paramIndex++),
          );
  ...
  static update<T>(driver: DatabaseDriver, table: string, data: Partial<T>): QueryBuilder<T> {
    ...
    const setParts = columns.map(
      (col, i) => `${quoteColumn(driver, col)} = ${builder._placeholderFn(i + 1)}`,
    );
