...
export class VaClient {
  ...
  constructor(private options: VaClientOptions) {
    ...
    if (options.models) {
      for (const [name, model] of Object.entries(options.models)) {
        ...
        const updatedAtField = override?.updatedAtField ?? findUpdatedAtField(options.schema, name);
        this.repositories.set(
          name,
          new Repository(this.driver, model.tableName, {
  ...
  private createDriverProxy(): DatabaseDriver {
    ...
    return {
      ...
      getPlaceholder: (index) =>
        pool['connections'][0]?.driver.getPlaceholder(index) ?? `$${index}`,
      getDialect: () => dialect,
    };
  }
  ...
  repository<T extends Record<string, any>>(name: string): Repository<T> {
    if (!this.repositories.has(name)) {
      throw new Error(
        `Repository "${name}" not registered. Register it in VaClient options.models.`,
      );
    }
  ...
  protected delegate<T extends ModelDelegate<any>>(
    ...
    if (!this.delegates.has(key)) {
      ...
      this.delegates.set(
        key,
        new Ctor(this.driver, meta, this.modelRegistry, quoterFor(this.options.driver)),
      );
    }
