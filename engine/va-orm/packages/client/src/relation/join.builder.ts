...
export class JoinBuilder {
  static joinCondition(
    ...
    if (meta.kind === 'many-to-one' || (meta.kind === 'one-to-one' && meta.isFkHolder)) {
      for (let i = 0; i < meta.fkFields.length; i++) {
        parts.push(
          `${quote(`${parentAlias}.${meta.fkFields[i]}`)} = ${quote(`${childAlias}.${meta.pkFields[i] ?? meta.pkFields[0]}`)}`,
        );
      }
    } else {
      for (let i = 0; i < meta.fkFields.length; i++) {
        parts.push(
          `${quote(`${childAlias}.${meta.fkFields[i]}`)} = ${quote(`${parentAlias}.${meta.pkFields[i] ?? meta.pkFields[0]}`)}`,
        );
      }
    }
  ...
  static joinClause(spec: JoinSpec): string[] {
    ...
    return [
      `${joinKeyword} ${quote(childTable)} AS ${childAlias} ON ${this.joinCondition(meta, parentAlias, childAlias, quote)}`,
    ];
  }
