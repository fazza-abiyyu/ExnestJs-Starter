...
function extractType(rest: string): string {
  const stop =
    /\b(not\s+null|null|primary\s+key|unique|default|check|references|collate|generated|constraint)\b/i;
...
function parseCreateIndex(statement: string): DesiredIndex | null {
  const m = statement.match(
    /CREATE\s+(UNIQUE\s+)?INDEX\s+(IF\s+NOT\s+EXISTS\s+)?("?)([\w$]+)\3\s+ON/i,
  );
  if (!m) return null;
