...
export class TypeGenerator {
    ...
    this.w.block(
      ...
      () => {
        for (const f of model.fields) {
          ...
          this.w.prop(
            f.name,
            `boolean | { where?: ${target}WhereInput; orderBy?: Record<string, 'asc' | 'desc'>; take?: number; skip?: number; select?: ${target}Select; include?: ${target}Include }`,
          );
        }
      },
      '};',
  ...
  private generateUniqueWhere(model: ModelBlock): void {
    this.w.block(
      `export type ${model.name}UniqueWhere = {`,
      () => {
        for (const f of model.fields) {
          if (this.isRelation(f)) continue;
