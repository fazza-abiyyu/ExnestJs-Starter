...
import type {
  ...
  FieldDefinition,
  FieldAttribute,
} from '../../../schema/src/schema.types.js';
...
import {
  ...
  columnNameOf,
  columnNameOfField,
} from '../../../schema/src/schema.mapping.js';
...
export class SqlGenerator {
  ...
  private generateTable(
    ...
    allModels: ModelBlock[],
  ): string {
    const tableName = tableNameOf(model);
    const scalarFields = model.fields.filter(
      (f) => !modelNames.has(f.type.replace('[]', '')) && !f.isIgnored,
    );
  ...
  private generateForeignKeyIndexes(
    model: ModelBlock,
    modelNames: Set<string>,
    allModels: ModelBlock[],
  ): string[] {
    ...
    for (const attr of model.attributes) {
      ...
      if (Array.isArray(fields)) {
        declared.add(
          fields
            .map((f) => columnNameOf(model, String(f)))
            .sort()
            .join('|'),
        );
      }
  ...
  private generateForeignKeys(
    ...
    for (const info of relations.values()) {
      ...
      const onDelete = info.onDelete
        ? ` ON DELETE ${ON_DELETE_SQL[info.onDelete] ?? info.onDelete}`
        : '';
      constraints.push(
