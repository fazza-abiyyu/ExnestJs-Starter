...
const COLUMN_TYPE_MAP: Array<[RegExp, string]> = [
  ...
  [
    /^((var)?char(acter)?( varying)?|text|citext|clob|nchar|nvarchar|varying character)$/i,
    'String',
  ],
  [/^(bool(ean)?)$/i, 'Boolean'],
...
function pascalCase(table: string): string {
  const singular =
    table.endsWith('s') && !table.endsWith('ss') && table.length > 3 ? table.slice(0, -1) : table;
  return singular
    .split('_')
...
export class SchemaIntrospector {
  ...
  private async introspectPostgresTable(schema: string, table: string): Promise<IntrospectedTable> {
    ...
    for (const row of indexResult.rows as Array<{ indexname: string; indexdef: string }>) {
      ...
      if (/UNIQUE/.test(row.indexdef ?? '') && uniqueKeySet.has([...cols].sort().join('|')))
        continue;
      indexes.push(cols);
    }
  ...
  private async introspectSqlite(): Promise<IntrospectedTable[]> {
    ...
    for (const t of tablesResult.rows) {
      ...
      for (const idx of indexResult.rows) {
        ...
        const cols = colsResult.rows.sort((a, b) => a.seqno - b.seqno).map((c) => c.name as string);
